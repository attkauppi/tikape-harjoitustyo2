...
//            kurssit.poistaOrvotKurssit();
        ...
        Spark.post("/kysymykset", (req, res) -> {
            ...
            // Näin saat aina viitattua oikeaan id-numeroon, kun viittaat
            // saveOrUpdaten-palauttamaan olioon.
            ...
            try {
                ...
                if ((req.queryParams("aihe").isEmpty() ||req.queryParams("kysymysteksti").isEmpty()) && !kurssi.getNimi().isEmpty()) {
       ...
       Spark.get("/kysymykset/:id", (req, res) -> {
        ...
        Spark.post("/kysymykset/:id", (req, res) -> {
            ...
            // Onko Aiheeseen liittyviä muita kysymyksiä? Jos ei ole, poistetaan aihe.
            if (onkoAihePoistettava==Boolean.FALSE) {
                aiheet.delete(aihe.getId());
                
                // Jääkö kurssille mitään muita aiheita? Jos ei, sekin poistetaan.
